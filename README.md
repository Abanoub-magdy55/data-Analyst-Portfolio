📊 Data Analytics Portfolio

Welcome to my Data Analytics & Business Intelligence portfolio.

I am a Data Analyst focused on transforming raw data into meaningful business insights using Power BI, Microsoft Excel, Power Query, and DAX.

This portfolio showcases projects covering sales analytics, customer churn, HR analytics, project management, social media analytics, financial transactions, and logistics.

🛠️ Tools & Skills

Power BI

Microsoft Excel

Power Query

DAX

Data Cleaning & Transformation

Data Visualization

Dashboard Development

KPI Analysis

Business Intelligence

Trend Analysis

Business Reporting

📊 Power BI Projects
1. Contoso Store BI Report

Tools: Power BI | DAX | Power Query

Interactive business intelligence dashboard analyzing sales performance across products, regions, and time.

Key Areas:

Sales & profit KPIs

Product performance

Regional analysis

Multi-year trends

Dynamic DAX measures

Interactive reporting

🔗 [View Project](./Power-BI-Projects/Contoso-Store-BI/)

2. Customer Churn Analysis

Tools: Power BI | DAX | Power Query

Analyzed telecom customer data to understand customer churn rates and identify patterns associated with customer attrition.

Key Areas:

Churn rate analysis

Customer segmentation

Churn trends

Customer behavior

Calculated columns

DAX measures

🔗 [View Project](./Power-BI-Projects/Customer-Churn-Analysis/)

3. Project Management Dashboard

Tools: Power BI | DAX | Power Query

Interactive dashboard analyzing project costs, benefits, income generation, regional performance, and project phases.

Key Areas:

Project KPIs

Cost reduction

Income generation

Project benefits

Regional performance

Cost-benefit analysis

🔗 [View Project](./Power-BI-Projects/Project-Management-Dashboard/)

4. Social Media Analytics Dashboard

Tools: Power BI | DAX | Power Query

Interactive dashboard monitoring social media performance across Facebook, Instagram, LinkedIn, and X.

Key Areas:

Impressions

Engagement

Audience growth

Response rates

Platform comparison

Weekly performance

🔗 [View Project](./Power-BI-Projects/Social-Media-Analytics/)

📈 Excel Projects

5. Tri-Stars Sales Performance Dashboard

Tools: Excel | Power Query

Sales dashboard analyzing sales performance and profitability.

Key KPIs:

Total Sales

Total Quantity

Total Profit

Average Discount

Profit Margin

🔗 [View Project](./Excel-Projects/Tri-Stars-Sales/)

6. Digital Wallet Transaction Dashboard

Tools: Excel | Power Query

Dashboard analyzing digital wallet transactions, revenue, payment categories, device usage, and transaction success rates.

🔗 [View Project](./Excel-Projects/Tri-Stars-Sales/)

7. Human Resources Analytics Dashboard

Tools: Excel | Power Query

Dashboard analyzing workforce distribution, salary, training hours, and employee performance.

🔗 View Project

8. Bike Sales Dashboard

Tools: Excel | Power Query

Dashboard analyzing bike sales trends, regional performance, and top-selling models.

🔗 View Project

9. Vrinda Store Dashboard

Tools: Excel | Power Query

Dashboard analyzing sales trends, customer demographics, product categories, regional performance, and order fulfillment.

🔗 View Project

10. Global Company Dashboard

Tools: Excel | Power Query

Logistics dashboard analyzing shipping performance, delivery metrics, and costs across multiple countries.

🔗 View Project

11. HR Report

Tools: Excel | Power Query

HR report analyzing employee productivity, experience levels, and salary distribution.

🔗 View Project

📌 Featured Skills

Data Analysis: Sales Analysis | Customer Churn | HR Analytics | Financial Analysis | Business Analytics

Business Intelligence: Power BI | Dashboard Development | KPI Reporting | Data Visualization

Data Preparation: Power Query | Data Cleaning | Data Transformation

Data Modeling & Analysis: DAX | Calculated Columns | Measures | Trend Analysis

📫 Contact

LinkedIn: [Your LinkedIn Profile](https://www.linkedin.com/in/abanoub-magdy-913510248/)

Email: abanoubmagdy869@gmail.com

⭐ Thank you for visiting my portfolio!
